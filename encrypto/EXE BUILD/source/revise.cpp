...
bool handshake_file_write(bool mem_val) {
	...
	if (outfile.is_open()){
		...
		cout << "Unable to create handshake file run the loader.exe" << endl;
		return false;
	}
	return false;
}
...
string handshake(){
    ...
    FILE *fp = popen(("serial.exe " + handshake_code).c_str(), "r");
    if (fp == NULL) {
        cout << "File doesnt contain handshake run the loader.exe exiting now... \n" << endl;
        sleep(5);
        return "false";
    }
...
bool handshake_rx_test(string handshake_py) {
    ...
    reversed_second = trim(reversed_second);
   	...
   	if (status == true or status == 1){
   		...
   		cout << "Handshake Failed run the loader.exe\nExiting now..." << endl;
   		sleep(2);
   		exit(1);
   	}
...
int loader(string name, int psk){
	...
	if (hand == false || hand == 0){
		cout << "\tHandshake Failed\nRetry Again run the loader.exe\nExiting Now......!\n" << endl;
		sleep(5);
		exit(0);
	}
	...
	if (id_auth == true){
		...
		if (psk_auth == true){
			...
			sprintf(buffer, "start cmd /c serial.exe %s %s", key.c_str(), script_ver_key.c_str());
			command = buffer;
